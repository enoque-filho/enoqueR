...
eda_tabela_dataset_geral = function(dados){
  ...
  for(i in 1:length(vars)){
    ...
    if(classe[i] %in% c("numeric","double","integer")){
      if(nrow(dados)<5000){
        W = shapiro.test(na.omit(dados[,vars[i]]))$statistic["W"]
        p = round(shapiro.test(dados[,vars[i]])$p.value,4)
        var_info[i] = paste0(ifelse(p>=0.05,"Normal","Não Normal"), " (W: ",round(W,5),", p: ",p,")")
      }
      else{
        ks = ks.test(unique(na.omit(dados[,vars[i]])), pnorm, mean(dados[,vars[i]], na.rm = TRUE), sd(dados[,vars[i]], na.rm = TRUE))$statistic["D"]
